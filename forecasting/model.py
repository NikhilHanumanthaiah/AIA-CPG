...
try:
    ...
    Prophet = None
    logger.warning("Prophet not installed or could not be imported. Running in Prophet-mock mode.")
...
class ForecastingService:
    ...
    def train_and_forecast(
        ...
        """
        logger.info("Training forecast model (Region: %s, Category: %s)", region, category)
        
        prepared_df = self.prepare_data(df)
        
        if len(prepared_df) < 2:
            logger.warning("Insufficient data points for forecasting. Returning mock forecast.")
        ...
        if Prophet is not None:
            try:
                ...
                logger.error("Error during Prophet execution: %s. Falling back to mock.", str(e))

        return self._generate_mock_forecast(periods, region, category)

    def evaluate_model(self, actual: pd.DataFrame, forecast: pd.DataFrame) -> Dict[str, float]:
        """
