...
def test_dynamic_upload_flow(client: TestClient, db_session: Session):
    ...
    """
    import json
    from database.models import StoreDimension, ProductDimension, SalesFact

    prod = ProductDimension(sku_id="SKU999", category="Beverages", brand="BrandA")
    store = StoreDimension(store_id="STORE999", region="Northeast", state="NY", city="New York")
    ...
    file_payload = {
        "file": ("sales_dynamic.csv", csv_content, "text/csv")
    }
    ...
    db_sales = db_session.query(SalesFact).filter(SalesFact.transaction_id == "TXNDYN001").all()
...
def test_upload_with_string_data_quality_rules(client: TestClient, db_session: Session):
    ...
    file_payload = {
        "file": ("customers_dq.csv", csv_content, "text/csv")
    }
...
def test_upload_with_multiple_rules_and_limits(client: TestClient, db_session: Session):
    ...
    store = StoreDimension(store_id="ST1", region="Northeast", state="NY", city="New York")
    db_session.add_all([prod, store])
    db_session.commit()
    ...
    file_payload = {
        "file": ("sales_multiple.csv", csv_content, "text/csv")
    }
