...
with tab_forecasts:
    ...
    with col_left:
        ...
        <div style="background-color: #f8fafc; padding: 1.25rem; border-radius: 8px; border: 1px solid #e2e8f0; margin-top: 1rem;">
            ...
            <ul style="margin-top: 0.5rem; margin-bottom: 0.5rem; font-size: 0.85rem; color: #475569; padding-left: 1.2rem;">
                <li><strong>X-Axis (Date):</strong> Timeline of projected sales dates over the slider-defined horizon.</li>
                <li><strong>Y-Axis (Revenue):</strong> Expected daily sales revenue in USD.</li>
                <li><strong>Blue Line:</strong> Daily revenue projections generated by the FB Prophet engine, incorporating historical trend changes and day-of-week demand seasonality.</li>
            </ul>
            <p style="font-size: 0.82rem; color: #64748b; margin-top: 0.5rem; line-height: 1.4;">
    ...
    with col_right:
        ...
        st.markdown("#### 2. AI Forecast Interpretation")
...
with tab_ai_insights:
    ...
    with st.expander("📋 View Database Tables & Columns Info"):
        st.markdown("""
        - **`fact_sales`**: transaction_id, transaction_timestamp, sku_id, store_id, customer_id, quantity, unit_price, revenue, currency
        - **`dim_product`**: sku_id, category, brand, package_size, launch_date
        - **`dim_store`**: store_id, region, state, city
        - **`customer_master`**: customer_id, customer_name, email, region
        """)
    ...
    if st.button("Execute AI Query Analysis", type="primary", use_container_width=True):
        if not user_query.strip():
            ...
            with st.spinner("Gemini is analyzing the database and writing SQL..."):
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        with col_tbl:
                            ...
                            if query_data["results"]:
                                df_res = pd.DataFrame(query_data["results"])
