...
class DataCleaner:
    ...
    def clean_sales(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            "Finished transformation: Cleaned sales transactions. Count: %d",
            len(cleaned_df),
        )
        return cleaned_df
    ...
    def clean_stores(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        cleaned_df = df.copy()
        cleaned_df["region"] = (
            cleaned_df["region"].fillna("Unknown Region").str.strip().str.title()
        )
        cleaned_df["state"] = (
            cleaned_df["state"].fillna("Unknown State").str.strip().str.upper()
        )
        cleaned_df["city"] = (
            cleaned_df["city"].fillna("Unknown City").str.strip().str.title()
        )
