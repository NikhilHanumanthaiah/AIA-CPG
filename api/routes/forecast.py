...
from database.models import ForecastResult, ProductDimension, SalesFact, StoreDimension
...
def run_forecast(
    ...
    data = [
        {"transaction_timestamp": r.transaction_timestamp, "revenue": float(r.revenue)}
    ...
    forecast_results = forecaster.train_and_forecast(
        df, periods=payload.days_to_predict, region=region_str, category=category_str
    )
    ...
    db.query(ForecastResult).filter(
        ForecastResult.region == region_str, ForecastResult.category == category_str
    ).delete()
