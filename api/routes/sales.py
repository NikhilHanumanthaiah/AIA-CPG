...
from database.models import SalesFact, StoreDimension, ProductDimension
...
def get_sales(
    ...
    """
    records = db.query(SalesFact).order_by(SalesFact.transaction_timestamp.desc()).offset(offset).limit(limit).all()
    return records
...
def get_sales_kpis(
    ...
    if category:
        query = query.join(ProductDimension).filter(ProductDimension.category == category)
    ...
    if category:
        regions_query = regions_query.join(ProductDimension).filter(ProductDimension.category == category)
...
def get_sales_trends(
    ...
    trends = query.group_by(cast(SalesFact.transaction_timestamp, Date)).order_by("sales_date").all()
    
    return [
        TrendItemResponse(date=str(row.sales_date), revenue=float(row.daily_revenue or 0.0))
        for row in trends
    ]
...
def get_regions(db: Session = Depends(get_db)):
    ...
    """
    regions = db.query(StoreDimension.region).distinct().order_by(StoreDimension.region).all()
...
def get_categories(db: Session = Depends(get_db)):
    ...
    """
    categories = db.query(ProductDimension.category).distinct().order_by(ProductDimension.category).all()
