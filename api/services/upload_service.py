...
class UploadService:
    ...
    def process_upload(
        ...
        if missing_cols:
            error_msg = (
                f"Missing mandatory columns for table '{target_table}': {missing_cols}"
            )
            logger.error(error_msg)
        ...
        try:
            ...
            existing_db_keys = {
                row[0] for row in existing_db_keys_query if row[0] is not None
            }
        except Exception as e:
        ...
        for idx, row in df.iterrows():
                ...
                try:
                    if expected_type in (int, float):
                        ...
                        if dt_parsed.tzinfo is not None:
                            cleaned_row[col_name] = dt_parsed.tz_convert(
                                None
                            ).to_pydatetime()
                        else:
            ...
            if rules_dict.get("currency_normalize") and target_table == "fact_sales":
                ...
                rates = {"EUR": 1.10, "CAD": 0.75, "GBP": 1.25, "USD": 1.0}
                rate = rates.get(curr, 1.0)
                if rate != 1.0:
                    if cleaned_row.get("unit_price") is not None:
                        cleaned_row["unit_price"] = round(
                            cleaned_row["unit_price"] * rate, 2
                        )
        ...
        if validation_errors:
            ...
            err_details = [
                f"Row {e['row_index']} ({e['column']}): {e['reason']}"
        ...
        if valid_records:
            try:
                ...
                db.commit()
                logger.info(
                    "Successfully loaded %d records into %s",
                    inserted_rows,
                    target_table,
                )
            except Exception as e:
